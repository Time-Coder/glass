...
class WriteProperty:
    ...
    def complex_build_wp_request(self, args, value, vendor_id):
        vendor_id = vendor_id
        addr = args[0]
        args = args[1:]
        obj_type, obj_inst, prop_id, _, priority, indx = self._parse_wp_args(args)
    ...
    def complex_write(self, args, value):
        vendor_id = 0
        timeout = 10
        ...
        try:
            ...
            iocb = IOCB(self.complex_build_wp_request(args, value, vendor_id=vendor_id))
    ...
    def _validate_value_vs_datatype(self, obj_type, prop_id, indx, vendor_id, value):
        ...
        elif str(type(value)) != str(datatype):
            if str(datatype) == "<class 'bacpypes.constructeddata.ArrayOf.<locals>.ArrayOf'>" and isinstance(value, list):
                from bacpypes.constructeddata import ArrayOf
                value = ArrayOf(type(value[0]))(value)
            else:
                raise TypeError(
