...
string str::replace_include(string content, const set<string>& include_paths)
{
	...
	while(true)
	{
		...
		{
			throw Exception("Error using #include: head file name must be envelop by <...> or \"...\"");
		}
		...
		{
			throw Exception(string("Error using #include: ") + end_char + " must be the end char.");
		}
		...
		if(!found)
		{
			throw Exception("Error in #include: file \"" + filename + "\" is not found!");
		}
	}
}
